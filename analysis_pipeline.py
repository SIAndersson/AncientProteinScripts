...
import torch
from Bio.PDB import DSSP, PDBIO, Atom, PDBParser, StructureBuilder
...
def write_coords_to_pdb(coords, sequence, filename):
    """
    Write coordinates to a valid PDB using Biopython objects (safe).
    coords: iterable of (x,y,z) (array-like or tensors)
    sequence: iterable of 3-letter residue names (e.g. 'ALA','GLY',...) or 1-letter (we expect 3-letter)
              If given 1-letter, the code will try to expand to 3-letter using a simple map.
    """
    # helper map for 1-letter → 3-letter (basic)
    ONE2THREE = {
        "A": "ALA",
        "R": "ARG",
        "N": "ASN",
    ...
    # normalize sequence to 3-letter codes
    seq3 = []
    for aa in sequence:
        if len(aa) == 1:
            seq3.append(ONE2THREE.get(aa.upper(), "UNK"))
        else:
            seq3.append(aa.upper())

    # Build structure
    structure_id = "generated"
    ...
    for coord, resname in zip(coords, seq3):
        # ensure coords are scalars
        x, y, z = coord[0], coord[1], coord[2]
        # create a residue and atom
        builder.init_seg("    ")
        ...
        builder.structure[0]["A"][resseq].add(atom)
        resseq += 1
...
def plot_structure_analysis(df, output_prefix, tm_threshold=0.5, rmsd_threshold=5.0):
    ...
    g = sns.displot(data=df, x="TM_score", hue="Fold", col="DSSP")
